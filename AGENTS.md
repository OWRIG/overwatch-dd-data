...
通过网易DD客户端的 datamsapi 接口获取守望先锋对战数据（对战历史、英雄/地图统计、单场详情等），供数据分析使用。
...
输出 `report.txt`（胜负驱动因子/英雄胜率/职责/地图/队友协同）+ `hero_stats.csv` + `map_stats.csv`。
...
- **`creds.json` 含你的私人 token，已被 `.gitignore` 排除，切勿提交或随 skill 分发出去。** 分发时只发 `SKILL.md` + `AGENTS.md` + `scripts/`（不含 creds.json 和任何 data.json）。
